...
# Real IPv6 with ~58ms RTT, no bandwidth cap
...
# Enable IP forwarding and NAT for CONNECT-IP
...
# Assign all addresses in the hopping prefix so the kernel can use them as source
...
sudo bash -c 'nohup /home/ubuntu/masque_server \
  ...
  $spa_flag \
  --stderrthreshold=0 -v=0 </dev/null >/home/ubuntu/server.log 2>&1 &'
sleep 2
...
sudo rm -f /tmp/capture.pcap

# Client hopping uses the last 8 bits of the primary address - no extra addresses needed

# Start pcap capture on outer QUIC traffic
sudo tcpdump -i ens5 -w /tmp/capture.pcap udp port 4433 -s 128 >/dev/null 2>&1 &
TCPDUMP_PID=\$!
sleep 1
...
CLIENT_EOF
  ...
  # Download pcap, logs, and keylog from client and proxy
  local prefix="$OUTPUT_DIR/validate_freq_${freq}_run${run}"
  scp_cmd ubuntu@"$CLIENT_IP":/tmp/capture.pcap "${prefix}.pcap" 2>/dev/null
  scp_cmd ubuntu@"$CLIENT_IP":/tmp/masque.log "${prefix}_client.log" 2>/dev/null
  ssh_cmd "$PROXY_IP" "sudo chmod 644 /home/ubuntu/server.log /home/ubuntu/sslkeylogfile.txt 2>/dev/null"
  scp_cmd ubuntu@"$PROXY_IP":/home/ubuntu/server.log "${prefix}_proxy.log" 2>/dev/null
  scp_cmd ubuntu@"$PROXY_IP":/home/ubuntu/sslkeylogfile.txt "${prefix}_keys.log" 2>/dev/null

  # Embed TLS keys into pcapng if editcap is available
  EDITCAP="/Applications/Wireshark.app/Contents/MacOS/editcap"
  if [ -f "$EDITCAP" ] && [ -f "${prefix}.pcap" ] && [ -f "${prefix}_keys.log" ]; then
    "$EDITCAP" --inject-secrets tls,"${prefix}_keys.log" "${prefix}.pcap" "${prefix}.pcapng" 2>/dev/null
    if [ -f "${prefix}.pcapng" ]; then
      rm -f "${prefix}.pcap" "${prefix}_keys.log"
    fi
  fi
}
  ...
  for mode in server client both; do
    ...
    for freq in "${FREQUENCIES[@]}"; do
      ...
      for run in $(seq 1 $NUM_RUNS); do
        ...
        # Restart proxy before each run to recover from any crashes
        ...
        # Override OUTPUT_DIR for the run_download function
        OUTPUT_DIR="$MODE_DIR"
