        ...
        if pcap_files:
            ...
            for json_path in json_files:
                ...
                if file_size_mb:
                    ...
                    run_goodputs.append(goodput_mbps)
                    run_throughputs.append(goodput_mbps)

                # Compute expected migration count from freq and duration
...
def make_normalized_plot(all_results, frequencies, labels, title_suffix, output_path_prefix):
    ...
    scenario_styles = {
        "server": {"color": "#2ecc71", "label": "Server-side SPA hopping"},
        "client": {"color": "#3498db", "label": "Client-side hopping"},
        "both": {"color": "#e74c3c", "label": "Both (server + client)"},
    }
    ...
    for i, scenario in enumerate(scenarios_present):
        ...
        # Use goodput if available (more meaningful when pcap includes idle time)
        ...
        for freq in freqs_nonzero:
            if freq in results:
                if use_goodput:
                    normalized.append(results[freq]["avg_goodput_mbps"] / baseline)
                    errors.append(results[freq]["avg_goodput_std"] / baseline)
                else:
                    normalized.append(results[freq]["avg_mbps"] / baseline)
                    errors.append(results[freq]["avg_mbps_std"] / baseline)
                mig_counts.append(
        ...
        for bar, mig in zip(bars, mig_counts):
            if bar.get_height() > 0 and mig > 0:
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.03,
                        f"{int(mig)}", ha="center", va="bottom", fontsize=7,
                        color=style["color"], fontweight="bold")

    ax.axhline(y=1.0, color="gray", linestyle="--", alpha=0.5, linewidth=1)
    ax.set_xticks(x_base)
    ax.set_xticklabels([str(f) for f in freqs_nonzero], fontsize=10)
    ax.set_xlabel("Migration Interval (ms)", fontsize=12)
    ax.set_ylabel("Normalized Throughput (relative to baseline)", fontsize=12)
    ax.set_title(f"QUIX: Normalized Throughput vs. Migration Interval\n{title_suffix}", fontsize=12)
    ax.legend(loc="lower right", fontsize=11)
    ax.grid(True, alpha=0.3, axis="y")
    ax.set_ylim(bottom=0.6, top=1.35)
...
def make_per_migration_plot(all_results, frequencies, title_suffix, output_path_prefix):
    ...
    for i, scenario in enumerate(scenarios_present):
        ...
        bp = ax.boxplot(
            ...
            flierprops={"marker": "o", "markersize": 5, "markerfacecolor": style["color"], "alpha": 0.7},
        )
        for patch in bp["boxes"]:
            patch.set_facecolor(style["color"])
            patch.set_alpha(0.7)

        ax.plot([], [], color=style["color"], linewidth=8, alpha=0.7, label=style["label"])

    ax.set_xticks(x_base)
    ax.set_xticklabels([str(f) for f in freqs_nonzero], fontsize=10)
    ax.set_xlabel("Migration Interval (ms)", fontsize=12)
    ax.set_ylabel("Throughput Loss per Migration (%)", fontsize=12)
    ax.set_title(f"QUIX: Throughput Cost per Address Change\n{title_suffix}", fontsize=12)
...
def main():
    ...
    for rtt in sorted(all_rtt_pathval_results.keys()):
        for pathval in pathval_modes:
            ...
            # --- Figure 1b: Throughput loss per migration (bar chart) ---
            make_per_migration_plot(
                all_results, frequencies,
                f"(RTT={rtt}ms, pathval={pathval})",
                os.path.join(figures_dir, f"throughput_per_migration_rtt{rtt}_{pathval}"))
            print(f"  Saved: figures/throughput_per_migration_rtt{rtt}_{pathval}.pdf")

            # Shared style definitions for consolidated figures
            scenario_styles = {
                "server": {"color": "#2ecc71", "marker": "o", "label": "Server-side SPA"},
                "client": {"color": "#3498db", "marker": "s", "label": "Client-side hopping"},
                "both": {"color": "#e74c3c", "marker": "^", "label": "Both (server + client)"},
            }
            freqs_nonzero = [f for f in frequencies if f > 0]
            x_positions = list(range(len(freqs_nonzero)))
            x_labels_str = [str(f) for f in freqs_nonzero]

            # --- Figure 3: Absolute Goodput (all modes, one plot) ---
            if file_size_mb:
                fig, ax = plt.subplots(1, 1, figsize=(10, 6))
                ...
                for scenario, results in all_results.items():
                    ...
                    errors = []
                    for freq in freqs_nonzero:
                        if freq in results and results[freq].get("avg_goodput_mbps", 0) > 0:
                            goodputs.append(results[freq]["avg_goodput_mbps"])
                            errors.append(results[freq]["avg_goodput_std"])
                        else:
                            goodputs.append(None)
                            errors.append(0)
                    valid_x = [x for x, v in zip(x_positions, goodputs) if v is not None]
                    valid_y = [v for v in goodputs if v is not None]
                    valid_err = [e for e, v in zip(errors, goodputs) if v is not None]
                    ax.errorbar(valid_x, valid_y, yerr=valid_err, capsize=4,
                ...
                # Add baseline band
                for scenario, results in all_results.items():
                    if results and 0 in results and results[0].get("all_goodput_mbps"):
                        baseline_max = max(results[0]["all_goodput_mbps"])
                        ax.axhline(y=baseline_max, color="gray", linestyle="--",
                                   alpha=0.4, linewidth=1)
                        break

                ax.set_xticks(x_positions)
                ax.set_xticklabels(x_labels_str, fontsize=10)
                ax.set_xlabel("Migration Interval (ms)", fontsize=12)
                ax.set_ylabel("Goodput (Mbps)", fontsize=12)
                ax.set_title(f"QUIX: Goodput vs. Migration Interval\n(RTT={rtt}ms, pathval={pathval})", fontsize=12)
                ax.legend(loc="lower right", fontsize=11)
                ax.grid(True, alpha=0.3)
                ax.set_ylim(bottom=0)
                plt.tight_layout()
                plt.savefig(os.path.join(figures_dir, f"goodput_rtt{rtt}_{pathval}.pdf"), bbox_inches="tight")
                plt.close()
                print(f"  Saved: figures/goodput_rtt{rtt}_{pathval}.pdf")

            # --- Figure 4: Migration count vs interval (all modes, one plot) ---
            fig, ax = plt.subplots(1, 1, figsize=(10, 6))
            freqs_nonzero = [f for f in frequencies if f > 0]
            x_positions = list(range(len(freqs_nonzero)))
            x_labels_str = [str(f) for f in freqs_nonzero]
            ...
            for scenario, results in all_results.items():
                ...
                for freq in freqs_nonzero:
                    if freq in results:
                        ...
                        mig_counts.append(None)
                valid_x = [x for x, v in zip(x_positions, mig_counts) if v is not None]
                valid_y = [v for v in mig_counts if v is not None]
                ax.plot(valid_x, valid_y, color=style["color"], marker=style["marker"],
                        linewidth=2, markersize=8, label=style["label"])

            ax.set_xticks(x_positions)
            ax.set_xticklabels(x_labels_str, fontsize=10)
            ax.set_xlabel("Migration Interval (ms)", fontsize=12)
            ax.set_ylabel("Total Migrations", fontsize=12)
            ax.set_title(f"QUIX: Address Changes vs. Migration Interval\n(RTT={rtt}ms, pathval={pathval})", fontsize=12)
            ax.legend(loc="upper right", fontsize=11)
            ax.grid(True, alpha=0.3)
            ax.set_ylim(bottom=0)
            plt.tight_layout()
            plt.savefig(os.path.join(figures_dir, f"migrations_rtt{rtt}_{pathval}.pdf"), bbox_inches="tight")
            plt.close()
            print(f"  Saved: figures/migrations_rtt{rtt}_{pathval}.pdf")
